...
import { getRegion, getRegionMetrics } from '@/services/api/regions';

const Regions = () => {
  const [selectedState, setSelectedState] = useState('maharashtra');

  // Fetch region data using React Query
  const { data: regionData, isLoading, isError, error } = useQuery({
    queryKey: ['region', selectedState],
    queryFn: () => getRegion(selectedState),
    staleTime: 5 * 60 * 1000, // 5 minutes
    retry: 1,
  });

  // Fetch region metrics using React Query
  const { data: metricsData } = useQuery({
    queryKey: ['region-metrics', selectedState],
    queryFn: () => getRegionMetrics(selectedState),
    staleTime: 5 * 60 * 1000, // 5 minutes
    retry: 1,
  });
    ...
    return (
      <div className="min-h-screen py-8 px-4 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <p className="text-muted-foreground mb-4">Failed to load regional data. Please try again later.</p>
  ...
  const currentData = region.id ? {
    name: region.name || 'Unknown Region',
    schemes: metrics.schemes_count || 0,
    totalBeneficiaries: metrics.total_beneficiaries ? `${(metrics.total_beneficiaries / 1000000).toFixed(1)}M` : '0',
    avgSentiment: metrics.avg_sentiment || 0,
    complaints: metrics.complaints_count || 0,
    successRate: metrics.success_rate || 0,
    topSchemes: metrics.top_schemes || [],
    recentUpdates: metrics.recent_updates || [],
    demographics: {
      rural: metrics.rural_percentage || 50,
      urban: metrics.urban_percentage || 50
    }
  } : mockStatesData[selectedState] || mockStatesData.maharashtra;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <Card className="mb-8">
          <CardHeader>
            ...
            <CardDescription>Based on beneficiary satisfaction and impact</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {currentData.topSchemes.map((scheme, index) => (
                <div key={index} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted transition-colors">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-muted-foreground">Beneficiaries: {scheme.beneficiaries}</div>
                  </div>
