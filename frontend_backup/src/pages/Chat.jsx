...
import { createDiscussion, addComment } from '@/services/api/discussions';
...
const Chat = () => {
  ...
  const handleSendMessage = async (message = inputMessage) => {
    ...
    try {
      ...
      if (!discussionId) {
        const discussionResponse = await createDiscussion({
          title: `Chat about: ${message.substring(0, 50)}...`,
          content: message
        });
        
        if (discussionResponse.success) {
          setDiscussionId(discussionResponse.data.id);
        }
      } else {
        // Add comment to existing discussion
        const commentResponse = await addComment(discussionId, {
          content: message
        });
        
        if (!commentResponse.success) {
          throw new Error(commentResponse.error?.message || 'Failed to send message');
        }
      }
      ...
      // Add bot response as a comment to the discussion
      if (discussionId) {
